...
parser.add_argument('--input_size', default=64, type=int, help='img crop size')
parser.add_argument('--image_size', default=64, type=int, help='ori img size')
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if i % args.print_freq == 0:
            line = 'Test: [{0}/{1}]\t' \
                   ...
                   'Loss {loss.val:.4f} ({loss.avg:.4f})\t' \
                   'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t' \
                   'Prec@5 {top2.val:.3f} ({top2.avg:.3f})'.format(i, len(val_loader), batch_time=batch_time,
